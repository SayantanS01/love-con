...
export default function DiaryEntryCard({ entry, index, onUpdated }: { entry: DiaryEntry, index: number, onUpdated: () => void }) {
  ...
  return (
    ...
    >
      <div className="surface-card p-10 shadow-premium group relative border-none overflow-hidden text-center flex flex-col items-center justify-center">
        {/* Mood Glow Decor */}
        {entry.moodTag && (
          <div className="mood-glow -top-10 -left-10" />
        )}
        ...
        <div className="space-y-8 w-full text-center flex flex-col items-center">
          <p className="text-2xl font-editorial leading-relaxed text-on-surface whitespace-pre-wrap max-w-2xl">
            {entry.content}
          </p>

          {entry.photoUrl && (
            <div className="max-w-3xl overflow-hidden rounded-xl bg-surface-container shadow-premium">
              <img src={entry.photoUrl} alt="Diary attachment" className="w-full h-auto grayscale-[0.3] group-hover:grayscale-0 transition-all duration-[2s]" />
            </div>
          )}
        </div>
              ...
              <div className="mt-10 pt-8 space-y-6 border-l border-primary/10 ml-6 pl-10 text-left">
                {replies.map((reply) => (
                  <div key={reply.id} className="relative">
                    ...
                    <div className="bg-surface-container-low/50 p-6 rounded-xl relative overflow-hidden">
                ...
                {showReplyInput && (
                  <form onSubmit={handleReply} className="flex gap-4 items-center">
                    <input 
                      ...
                      className="flex-1 bg-surface-container-low border-none rounded-full px-8 py-4 text-sm focus:ring-2 focus:ring-primary/10 transition-all"
                    />
                    <button 
                      type="submit"
